...
export default function CosasHacer() {
  const isAdmin = localStorage.getItem("isAdmin") === "true";
  const [cosas, setCosas] = useState<any[]>([]);
  ...
  const fetchData = async () => {
    const snapshot = await getDocs(collection(db, "cosasHacer"));
    setCosas(snapshot.docs.map(doc => ({ id: doc.id, ...doc.data() })));
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!nombre || !concepto || !descripcion || !imagenBase64) {
      alert("Completa todos los campos");
      return;
    }
    await addDoc(collection(db, "cosasHacer"), {
      nombre, concepto, descripcion, imagen: imagenBase64
    });
    alert("Lugar agregado ✅");
    setNombre(""); setConcepto(""); setDescripcion(""); setImagenBase64("");
    setShowForm(false);
    fetchData();
  };
      ...
      <div style={{ marginTop: "20px" }}>
        {cosas.map((c) => (
          <div key={c.id} style={{ border: "1px solid #ccc", padding: "10px", marginBottom: "10px" }}>
            <h3>{c.nombre}</h3>
            <p><b>Concepto:</b> {c.concepto}</p>
            <p>{c.descripcion}</p>
            {c.imagen && <img src={c.imagen} alt={c.nombre} style={{ maxWidth: "200px" }} />}
          </div>
        ))}
      </div>
    </div>
